...
module.exports = {
    ...
    */
    newMessage: async (sender, relationship, message, timestamp) => {
        ...
        if(status.get(messageRelationship.status.name) != status.APPROVED) throw new UnprocessibleRequest(`Only Active Relationships - ${messageRelationship.status.name} can add messages to the channel`);
        let found = false;
